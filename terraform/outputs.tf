...
# IP Pública de la instancia EC2 (Si es una EC2 estática)
output "instance_public_ip" {
  value       = aws_instance.infra_server.public_ip
  description = "La IP pública de la instancia EC2"
}

# IP Privada de la instancia EC2 (Si es una EC2 estática)
output "instance_private_ip" {
  value       = aws_instance.infra_server.private_ip
  description = "La IP privada de la instancia EC2"
}

# IP Pública de las instancias en el Auto Scaling Group
output "asg_instances_public_ips" {
  value = [for instance in data.aws_instances.asg_instances.instances : instance.public_ip]
  description = "Las IPs públicas de las instancias en el Auto Scaling Group"
}

# IP Privada de las instancias en el Auto Scaling Group
output "asg_instances_private_ips" {
  value = [for instance in data.aws_instances.asg_instances.instances : instance.private_ip]
